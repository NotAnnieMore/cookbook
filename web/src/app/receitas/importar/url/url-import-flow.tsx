    ...
    <section className="mx-auto max-w-5xl px-5 pb-24 sm:px-8">
      <form action={action} className="rounded-[2rem_2rem_4.5rem_2rem] bg-[#FFFCF6] p-6 shadow-[0_10px_0_#E6DED2] sm:p-8">
        ...
        <details className="mt-4 rounded-2xl bg-[#F8F4EC] px-4 py-3 text-sm text-[#6F6860]">
          ...
          <p className="mt-3 leading-6">Procuramos primeiro dados Schema.org próprios para receitas. Quando não existem, tentamos interpretar o conteúdo visível e os metadados públicos. Sites privados, páginas que exigem login ou bloqueiam automação podem não funcionar.</p>
        </details>
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button type="submit" disabled={pending} className="min-h-14 rounded-full bg-[#F36F56] px-7 text-base font-extrabold text-white shadow-[0_6px_0_#D94F38] disabled:cursor-wait disabled:opacity-60">{pending ? "A visitar e analisar…" : "Criar preview"}</button>
        </div>
      </form>
    </section>
  );
