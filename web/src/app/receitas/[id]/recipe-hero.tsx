...
function HeroSkeleton() {
  return (
    <div className="grid grid-rows-[30rem_16rem] animate-pulse bg-[#E8E0D4] sm:grid-rows-[34rem_22rem] lg:h-[33rem] lg:grid-cols-[1.05fr_.95fr] lg:grid-rows-none" role="status" aria-label="A preparar a receita">
      <div className="flex min-h-0 flex-col justify-center overflow-hidden p-7 sm:p-11 lg:p-14">
        ...
        <div className="mt-7 h-12 w-5/6 rounded-full bg-[#CEC3B5]" />
        <div className="mt-3 h-12 w-2/3 rounded-full bg-[#CEC3B5]" />
        <div className="mt-7 h-4 w-full max-w-xl rounded-full bg-[#D8CFC3]" />
  ...
  return (
    <section className="overflow-hidden rounded-[2rem_2rem_5rem_2rem] text-white shadow-[0_14px_0_#E3DCD0] transition-colors duration-500" style={{ backgroundColor: palette.colour }} aria-busy={!palette.isReady}>
      {!palette.isReady ? (
        ...
        <div className="grid grid-rows-[30rem_16rem] animate-[cookbook-reveal_.28s_ease-out] sm:grid-rows-[34rem_22rem] lg:h-[33rem] lg:grid-cols-[1.05fr_.95fr] lg:grid-rows-none">
          <div
          ...
          >
            <div className="relative z-10">
              ...
              <h1 className="mt-4 line-clamp-2 max-w-3xl font-serif text-4xl font-black leading-[.98] tracking-[-.05em] sm:text-5xl lg:text-[3.4rem]">{title}</h1>
              ...
              <dl className="mt-9 flex flex-wrap gap-x-8 gap-y-4 border-t border-white/20 pt-6">
                ...
                {servings ? <div><dt className="text-[11px] font-extrabold uppercase tracking-wider text-white/55">Rende</dt><dd className="mt-1 font-serif text-lg font-bold">{servings}</dd></div> : null}
                {difficulty ? <div><dt className="text-[11px] font-extrabold uppercase tracking-wider text-white/55">Dificuldade</dt><dd className="mt-1 font-serif text-lg font-bold">{difficulty}</dd></div> : null}
              </dl>
            </div>
          </div>
